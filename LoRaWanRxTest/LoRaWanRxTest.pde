...
void loop() 
{
  ...
  if (error == 0)
  {
    ...
    USB.println(sizeof(LoRaWAN._buffer));
    USB.print(F("packet: "));
    for (int i =0 ; i <sizeof(LoRaWAN._buffer); i++)
    {
      USB.print(LoRaWAN._buffer[i]);
    }

    

     USB.print(F("dev adr: "));
    for (int i =0 ; i <sizeof(LoRaWAN._devAddr); i++)
    {
      USB.print(LoRaWAN._devAddr[i]);
    }

 USB.print(F("_data: "));
    for (int i =0 ; i <sizeof(LoRaWAN._data); i++)
    {
      USB.print(LoRaWAN._data[i]);
    }
    char new_buffer  [200];
    
    Utils.hex2str(LoRaWAN._buffer, new_buffer);
    USB.print(F("Printing--"));
    USB.println(  new_buffer);
    USB.print(F("length: "));
